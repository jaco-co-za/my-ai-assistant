...
function extractGradeCandidates(text) {
  ...
  const wordPattern = new RegExp(`\\b(?:grade|graad|gr)\\s*\\.?\\s*(${gradeWords})\\b`, "g");
  const inverseWordPattern = new RegExp(`\\b(${gradeWords})\\s*(?:grade|graad)\\b`, "g");
  const compactWordPattern = new RegExp(`\\b(${gradeWords})(?:grade|graad)(?=\\b|[a-z])`, "g");
  const wordPatterns = [wordPattern, inverseWordPattern, compactWordPattern];
  for (const pattern of wordPatterns) {
    let match = null;
    while ((match = pattern.exec(normalized)) !== null) {
      const grade = wordMap.get(match[1]) ?? null;
